    ...
    this.authService.checkUsername(user).subscribe(res => {
      ...
      if (res.exists == false) {
        ...
        this.authService.registerUser(user).subscribe(data => {
          if (data.success) {
            this.flashmessage.show('Регистрация успешна', { cssClass: 'alert-success', timeout: 3000 });
        ...
        this.flashmessage.show('Такой логин уже используется', { cssClass: 'alert-danger', timeout: 3000 });
      }
    });
  }
}
